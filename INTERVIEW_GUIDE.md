...
- **ClusterIP Service** (`raft-client`) — the normal kind, one stable address for external clients; any node can answer a client request, so load-balancing here is fine.
...
`/health` (liveness) is intentionally unconditional — a node mid-election is alive and must not be restarted, since that would throw away in-memory election state and potentially force an unnecessary failover. `/ready` (readiness) is role-aware, reflecting real Raft state, not just "the process started" — verified live: a recreated pod sat at `0/1` ready for ~4 seconds after its container was already `Running`, before flipping to `1/1` once it had genuinely rejoined the cluster.
