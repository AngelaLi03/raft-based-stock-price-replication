...
This is the one piece of this migration with no Compose equivalent at all — because Compose has no concept of a *voluntary* disruption to budget against in the first place. A PodDisruptionBudget (PDB) tells Kubernetes: don't let a voluntary action that goes through the Eviction API — `kubectl drain`, a manual eviction, a cluster-autoscaler decision — push the number of healthy pods matching this selector below N, even if the action would otherwise be allowed. `gen_k8s_manifests.py` sets `minAvailable` to `quorum(num_nodes)` — literally the same `(N // 2) + 1` formula `raft/node.py`'s commit logic uses to decide when a write is safely durable. For the 5-node local default, that's `minAvailable: 3`.

This is the sense in which the PDB *is the deployment layer learning Raft's own quorum rule*, not a separately-invented safety number that happens to look similar: without it, nothing stops a node drain or a manual eviction from taking 3 of 5 pods down at once — which wouldn't just be an availability blip, it would drop the cluster below the number of nodes it needs to elect a leader or commit anything at all, an outage indistinguishable from an actual failure, except this one would have been self-inflicted by routine maintenance that Kubernetes considered perfectly safe by its own (quorum-blind) defaults. (A *rolling update* is a separate case, covered below — the PDB only governs the Eviction API, which `kubectl rollout restart` never goes through.)
...
`/ready` is narrower than "caught up enough to usefully serve traffic" might suggest: `RaftNode.__init__` sets `self.state = RaftState.FOLLOWER` immediately, before crash recovery or any peer contact, and the metrics server starts before `raft_node.start()` runs — so `/ready` returns 200 from the moment the process can answer at all, and returns 503 only during the brief window a node is actively `CANDIDATE` mid-election. Verified live: a deleted-and-recreated `raft-node-3` sat at `READY 0/1` for about 4 seconds after its container was already `Running`, before flipping to `1/1`. That gap is fully explained by `initialDelaySeconds: 2` plus normal container/process boot time — it doesn't by itself prove `/ready` is reading real Raft state, since an endpoint that unconditionally returned 200 would have produced the same observation. What *is* verified: `/ready` correctly returns 503 while a node is mid-election, which the old unconditional `/health` never did.
