...
**Answer**: "The deployment layer has to know the same quorum math the algorithm does. My PodDisruptionBudget's `minAvailable` is generated as `quorum(N)` — same formula Raft uses to commit — so a voluntary eviction (drain, autoscaler, manual) can never be allowed to drop below majority. Rolling updates are a separate mechanism: the StatefulSet deletes pods directly rather than evicting them, so the PDB doesn't govern them at all — what keeps a routine rollout safe is the default one-ordinal-at-a-time update strategy. Live-verified: a 3rd eviction was refused by the API server directly, and a full rolling restart never dropped ready pods below 4 of 5."
