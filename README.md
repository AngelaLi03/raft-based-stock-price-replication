...
- **Full test suite passes**: 169 tests (129 pre-existing/fixed + 11 for snapshotting + 3 for BatchPutPrice + 5 for the gRPC/metrics-server layer + 5 for benchmark.py/chaos_test.py importability + 4 for the InstallSnapshot boundary and next_index overshoot bugs + 1 for the node-role metric + 3 for role/state metrics on followers + 1 for election-duration recording + 3 for the candidate-role callback wiring + 4 for the RPC-timeout/heartbeat-cadence/premature-ack bugs found via live chaos testing), verified by actually running `pytest` after installing `requirements.txt` into a working venv — not just read for plausibility
...
15. **Grafana/Prometheus monitoring stack added, then hardened through a final whole-branch review and a live demo — 6 findings, all fixed and re-verified live**:
    - **Two real metrics gaps closed first**: no metric reported which Raft role a node was in (added `raft_node_role` gauge), and only the *leader* ever refreshed its own state gauges (`update_node_state()` was only called from the leader-only heartbeat loop) — followers never reported `commit_index`/`current_term`/`last_applied`/`log_length` at all. Fixed with a role-independent `_metrics_tick_loop()`, started in `start()` and cancelled in `stop()`, plus immediate updates on every role transition.
    - **Per-Follower Commit Lag panel was permanently empty**: `max(raft_commit_index) - raft_commit_index` — bare `max()` drops all labels, so the label-less left side can never vector-match the labeled right side. Fixed to `scalar(max(raft_commit_index)) - raft_commit_index`.
    - **Election Duration panel could never have data**: `record_election()` existed in `raft/prometheus_metrics.py` with zero callers anywhere in the codebase. Fixed by instrumenting `ElectionManager._start_election()` to time the whole election (start to resolution, win or lose) and call it.
    - **`raft_node_role` could never emit "candidate" (1)**: `RaftNode.state` (what the metrics code reads) is only ever assigned `FOLLOWER`/`LEADER`; the `CANDIDATE` transition lived only on the separate `ElectionManager.state` object with no callback back to `RaftNode`. Fixed by adding an optional `become_candidate_callback` (mirrors the existing leader/follower callback pattern, defaults to `None` so it doesn't break existing callers) and a new `RaftNode._on_become_candidate()`.
    - **Neither alert rule set `noDataState`**, defaulting to Grafana's `NoData` — meaning a *total* cluster outage (metric doesn't exist at all, not just no leader among nodes that are up) would not fire the "no leader" alert, the exact worst case it should catch. Fixed: `noDataState: Alerting` on the no-leader rule, `noDataState: OK` on the flapping rule.
    - **Node count (15) hardcoded** in `ops/monitoring/prometheus/prometheus.yml` and the dashboard's quorum panel while `gen_docker_compose.py --nodes N` stays parametric — not fixed (would need the generator to also emit the monitoring config), documented with inline comments instead.
    - All fixes live-verified against the running 15-node cluster + Prometheus + Grafana (not just re-provisioned and assumed correct): commit-lag panel returns real data, election-duration metric increments after a real election, candidate-role wiring confirmed via unit tests (elections resolve in ~150-300ms, faster than the 5s scrape interval, so catching `role=candidate` in a live Prometheus query is expected to be difficult/flaky — the unit tests are the reliable proof here), both `noDataState` values confirmed via Grafana's provisioning API. 4 new tests (1 for election-duration recording, 3 for the candidate-role callback).
...
17. **Three real bugs found by running `chaos_test.py` live against the 15-node cluster, all with regression tests, all fixed**:
    - **Critical: unbounded peer/client RPCs could hang a node's election or heartbeat loop forever.** None of `raft/node.py`'s gRPC calls (`RequestVote`, `AppendEntries`, `InstallSnapshot`) or `scripts/kvctl.py`'s `RaftClient` carried a timeout. A peer that accepts the TCP connection but never responds (observed live during a container restart) hung the caller indefinitely instead of failing fast — and since `_start_election`/`_heartbeat_loop` each batch every peer's RPC into one `asyncio.gather()` and wait for all of them, one stuck peer froze that node's *entire* election or heartbeat loop, not just replication to that one peer. Live symptoms: a candidate stuck in `CANDIDATE` state for 6+ minutes, a leader whose heartbeat loop went completely silent mid-term, and `chaos_test.py` itself (which uses `RaftClient` to poll cluster state) hanging for 25+ minutes on a live run. Fixed with a 2-second `RPC_TIMEOUT_SECONDS` on both sides; 2 regression tests reproduce the hang against a real TCP listener that accepts but never responds (not a mock) and prove the fix bounds the call.
...
5. **A leader isolated from the majority (but still running) never steps down on its own**: this is correct, standard Raft — the base algorithm has no lease/step-down-on-isolation mechanism — but it's worth knowing explicitly rather than discovering it live: a leader cut off from enough peers to lose quorum keeps believing and reporting itself as leader (`raft_node_role` stays `2`) indefinitely; it simply can't get anything committed anymore. Found while building the "no leader elected" Grafana alert's demo scenario (see "Fixed this session" #16) — the alert correctly does not fire in this case, since a node genuinely still claims leadership. A production system wanting to detect "isolated leader, not just no leader" would need a separate signal (e.g. commit-lag-behind-cluster, or a leader lease with an explicit self-step-down timeout) — not implemented here.
6. **`chaos_test.py` itself hung once, unexplained, after the fixes in "Fixed this session" #17 above**: mid-`test_follower_catchup`, after a normal, instant "connection refused" for an already-stopped node, the process went completely silent — 0% CPU, no open network connections, no child processes — for 25+ minutes before being killed. This does *not* match the RPC-hang bug already fixed above (that would show a pending, open connection; this showed none at all), so it's more likely a bug in the test script's own async control flow or a Docker/host hiccup from heavy container churn, not a library correctness issue — but it wasn't reproduced in isolation and the root cause is genuinely unknown. Whoever picks this up next: try to reproduce with each `await` in `_get_leader()`/`_write_test_data()` individually wrapped in `asyncio.wait_for()` to isolate exactly which call (if any) never returns, rather than re-running the full 3-test suite and waiting on it live.
...
- **Grafana**: `http://localhost:3000` (`admin`/`admin`) — "Cluster Health" dashboard auto-loads with 6 panels, all live-verified to show real data: current role per node (follower/candidate/leader), current term per node, leader changes over time, election duration, per-follower commit lag, and quorum health.
- **Alerts** (visual-only in Grafana's Alerting UI, no external delivery in v1), both live-verified to actually fire and resolve: no leader for >10s, and leader changing more than twice in 60s (flapping). See Known Issue #5 for what "no leader" does and doesn't detect (an isolated-but-still-running leader is not the same as no leader).
...
**`scripts/dashboard_demo.py`** runs a full guided sequence against the live cluster (write burst, leader failover, follower catch-up, quorum loss, leader flapping), paced with real sleeps so each event is watchable on the dashboard and Grafana's Alerting page as it happens — `PYTHONPATH=. python3 scripts/dashboard_demo.py` for the whole sequence, or `--scenario <name>` for one (`baseline`, `write-burst`, `leader-failover`, `follower-catchup`, `quorum-loss`, `leader-flapping`, `final-check`).
